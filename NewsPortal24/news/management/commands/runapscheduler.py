...
from NewsPortal24.news.models import Post, Subscriber
...
def send_weekly_mails():
    ...
    new_posts = Post.objects.filter(time_in__gte=one_week_ago)
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        scheduler.add_job(
            send_weekly_mails,
            trigger=CronTrigger( day_of_week='sat', hour=17, minute=43),   #day_of_week=''
            id="send_weekly_mails",
            max_instances=1,
            replace_existing=True,
        )
        logger.info("Добавлена работа send_weekly_mails..")
        scheduler.add_job(
